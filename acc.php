...
$check_query = mysqli_query($link,"select * from acc where acc='$username' and password='$userpassword'");
	//讀取會員資料
...
/*$check_query = mysqli_query($link,"select * from con where acc='$username'");
	//讀取會員資料
	if($result=mysqli_fetch_assoc($check_query)){ 
		print(json_encode($result)); //印出JSON格式
	}else{
...
///if($check_query = mysqli_query($link,"select * from con ")){

//$result = mysqli_query($link,"replace INTO con(acc,a1,a2,a3,a4) VALUES('$name','$a1', '$a2', '$a3', '$a4')");
